...
/// The connection a [GatedDataSource] hands out: every method delegates to the
/// pool's own connection; [#close()] returns it to the pool and **then**
/// releases the gate permit, exactly once — a second `close()` is a no-op
/// (`docs/spec/admission.md` §1). `unwrap` passes through, so the deadline's
/// `unwrap(PgConnection.class).cancelQuery()` reaches the driver. Generated
/// from `java.sql.Connection` + `java.sql.Wrapper`; do not hand-edit the
/// delegating methods.
