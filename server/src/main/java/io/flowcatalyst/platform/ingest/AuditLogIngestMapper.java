...
public final class AuditLogIngestMapper {
    ...
    /// @param principalId       the item's own `principalId`, or `null` if absent
    /// @param performedAtRaw    the item's own `performedAt` (RFC 3339 text), or `null` if absent
    /// @param callerPrincipalId the authenticated caller's principal id (spec §4.3: the default actor)
    public static AuditLog toLog(String entityType, String entityId, String operation, JsonNode operationData,
        ...
        String actingPrincipal = blank(principalId) == null ? callerPrincipalId : principalId;
