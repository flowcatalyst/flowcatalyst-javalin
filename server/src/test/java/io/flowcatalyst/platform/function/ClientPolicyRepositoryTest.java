...
class ClientPolicyRepositoryTest {
    ...
    @Test
    void findByClientRoundTripsSignersAndCeilings() {
        String clientId = fresh();
        Instant now = Instant.now();
        ClientPolicy p = new ClientPolicy(clientId,
                List.of(new ClientPolicy.SignerRule("https://issuer", "subject-1", Set.of(Runtime.JVM, Runtime.WASM))),
                1000, 10, 64, 4, now, now);
        persist(p);

        ClientPolicy reloaded = REPO.findByClient(clientId).orElseThrow();
        ...
        ClientPolicy updated = new ClientPolicy(clientId, List.of(), null, null, null, null, now, Instant.now());
        persist(updated);
        ClientPolicy reloadedAgain = REPO.findByClient(clientId).orElseThrow();
        assertThat(reloadedAgain.signers()).isEmpty();
        assertThat(reloadedAgain.maxDurationMs()).isNull();

        assertThat(REPO.findByClient("clt_doesnotexist")).isEmpty();
    }
    ...
    void signersReadsAsEmptyWhenTheColumnIsAnEmptyArray() throws SQLException {
        ...
        assertThat(REPO.findByClient(clientId).orElseThrow().signers()).isEmpty();
    }
}
