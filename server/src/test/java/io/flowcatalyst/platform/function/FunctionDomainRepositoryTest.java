...
class FunctionDomainRepositoryTest {
    ...
    void claimFindAndVerifyRoundTrip() {
        ...
        FunctionDomain claimed = persist(FunctionDomain.claim("clt_1", host, "token-" + fresh(), Instant.now()));
        ...
        assertThat(REPO.listByClient("clt_1")).extracting(FunctionDomain::id).contains(claimed.id());
        ...
        assertThat(reloadedVerified.usableBy("clt_1")).isTrue();
    ...
    void persistOnlyEverChangesVerifiedAt() {
        ...
        FunctionDomain claimed = persist(FunctionDomain.claim("clt_1", host, "original-token", Instant.now()));

        FunctionDomain tampered = new FunctionDomain(claimed.id(), "clt_other", host, "different-token",
                claimed.verification(), claimed.createdAt());
        persist(tampered);

        FunctionDomain reloaded = REPO.findById(claimed.id()).orElseThrow();
        assertThat(reloaded.clientId()).as("SET list is verified_at only").isEqualTo("clt_1");
    ...
    void deleteRemovesTheRow() {
        ...
        FunctionDomain claimed = persist(FunctionDomain.claim("clt_1", host, "token", Instant.now()));
        UOW.inTransaction(tx -> {
