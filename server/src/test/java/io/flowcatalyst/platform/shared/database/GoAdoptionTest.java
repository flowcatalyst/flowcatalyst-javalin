...
class GoAdoptionTest {
    ...
    void goDatabaseIsBaselinedAtV1AndGooseTableIsLeftAlone() throws Exception {
        ...
        // portal_apps, spec `portal-apps.md`), V10's effect (054
        // dispatch_job_queue, spec `dispatch-job-priority.md`) AND V12's
        // effect (056 connection_application_scope, spec
        // `code-first-connections.md`): each of those is idempotent
        // (IF NOT EXISTS / pg_constraint guards / IF NOT EXISTS indexes) and a
        // no-op here; V8 (`mail_outbox`) is a genuinely new, Java-only table
        // this Go-adopted database does not have yet (spec `mail-outbox.md`,
        ...
        // V2..V7, V9, V10 and V12 still change nothing (flyway_schema_history
        // is ignored by the fingerprint); V8 is the one genuine addition
        ...
        assertThat(afterWithoutMailOutbox)
                .as("V2..V7, V9, V10 and V12 change nothing beyond V8's own new mail_outbox table")
