...
import io.flowcatalyst.fnhost.wasm.WasmRustFixtures;
...
/// The committed `function-hello-rust` module (`docs/spec/function-rust-guest.md`
/// §4) through the REAL reconciler and listener — the Rust twin of
/// [WasmFunctionHelloJsTest]. Every endpoint answers as the JS/Java
/// `examples/function-hello*` do for the same inputs, and an `http.request` to
/// a host outside `manifest.httpAllow` is a clean, guest-visible denial rather
/// than a trap.
class WasmFunctionHelloRustTest {

    private static final FunctionAddress ADDR = FnHttpTestSupport.ADDR_A;
    private static final String WEBHOOK_SECRET = "rust-hello-webhook-secret";
    ...
    /// The manifest `examples/function-hello-rust/manifest.json` ships,
    /// reproduced here (rather than read from the file — the same convention
    /// [WasmFunctionHelloJsTest] and [io.flowcatalyst.fnhost.wasm.WasmFixtures#manifest]
    /// use) so this test controls `httpAllow` for the denial case below.
    ...
    private static DesiredDocument.Entry entry(Path wasm) {
        return new DesiredDocument.Entry(ADDR, "fnc_hello_rust", "v1", 1, DesiredDocument.Role.LIVE,
    ...
    @Test
    void healthzAnswersOkJustLikeTheJavaAndJsHelloFunctions(@TempDir Path dir) {
        Path wasm = WasmRustFixtures.guest(dir);
    ...
    void helloChecksThePermissionBeforeGreetingTheCaller(@TempDir Path dir) throws Exception {
        ...
        try (TestJwks jwks = new TestJwks()) {
            ...
            try (var h = FnHttpTestSupport.start(dir, FnHttpTestSupport.oneFunction(entry(wasm)), 50, options)) {
                String allowed = jwks.mint("prn_rust_1", "SERVICE", "CLIENT", GREET_PERMISSION, List.of("clt_1"),
                ...
                assertThat(okBody.path("principalId").asString()).isEqualTo("prn_rust_1");

                // mutant: skip the has_permission check — a caller without the application's own
                // permission must never reach the greeting.
                String noPermission = jwks.mint("prn_rust_2", "SERVICE", "CLIENT", "hello:greeting:other",
    ...
    void greetingRequestedReadsConfigAndSecretPresenceThenEmitsAndAcks(@TempDir Path dir) {
        ...
        try (var h = FnHttpTestSupport.start(dir, FnHttpTestSupport.oneFunction(entry(wasm)))) {
            ...
            assertThat(resp.statusCode()).as(text(resp)).isEqualTo(200);
            assertThat(resp.body()).as("FunctionResult::ok() is an empty body — the ack row").isEmpty();
    ...
    void proxyReachesAnAllowlistedLoopbackServerAndDeniesAHostOutsideHttpAllow(@TempDir Path dir) throws Exception {
        ...
        try {
            ...
            try (var h = FnHttpTestSupport.start(dir, FnHttpTestSupport.oneFunction(entry(wasm)))) {
                ...
                // docs/spec/function-rust-guest.md §4 names.
