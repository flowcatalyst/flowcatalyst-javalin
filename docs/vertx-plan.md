...
rounds): with the owner's request-level admission — per-group FIFO queue + N virtual-thread
workers, `NO_DB` routes unbounded — the Vert.x server reaches **98% of Go's throughput at two
CPUs** (2,496 vs 2,550 req/s) with Go's spread and a p99 of 556 vs 440 ms on a 400 ms queueing
floor; pinned cores 99%. One CPU stays at 66% (JIT and collector on the same core; the native
image is slower still). The listener choice itself is within 2%; the semaphore gate variants and
