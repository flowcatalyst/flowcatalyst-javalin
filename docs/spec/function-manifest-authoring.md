...
- Writes a starter project derived from `examples/function-hello`: `pom.xml` depending on
  `flowcatalyst-function-api`, which fcdev ships with the scaffold under `lib/m2` (a project-local file repository the pom declares), versioned as the fcdev that carried it — owner ruling 2026-09-24, one handler class in `--package`
