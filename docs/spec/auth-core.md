...
| 2 | Access-token lifetime | 3600 s | AS:204, WS:86, responses `expires_in: 3600` hard-coded TK:485/607/708, LE:326, SAA:342 | C | Load-bearing for SDKs; note the response literal is **not** derived from config — if config changed, `expires_in` would lie. **Q15** |
...
| Q4 | Keep `/oauth/token`, `/oauth/introspect`, `/oauth/revoke`, `/oauth/userinfo`, discovery and JWKS **behind** the Authenticator (so a bad explicit Bearer is 401'd by the middleware with `Bearer error="invalid_token"` before the handler runs, and a stale cookie is ignored)? | yes |
| Q5 | Keep skipping the per-client rate buckets when the client authenticates with Basic only (no body `client_id`)? | yes |
| Q6 | Keep `client_credentials` reading `client_id`/`client_secret` **only from the body** (Basic auth refused with "Missing client_id") although discovery advertises `client_secret_basic`? | yes |
| Q7 | Keep 401 (not RFC 6749's 400) for `invalid_grant` on the refresh grant? | yes |
| Q8 | Keep advertising `RS256` in discovery even when running HS256? | yes |
| Q9 | Keep `defaultScopes` as a string on create and an array on update/response? | yes |
| Q10 | Keep 400 `MINT_FAILED` (not 500) when the cookie cannot be minted after a correct password? | yes |
