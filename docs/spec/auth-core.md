...
| Q14 | **RULED (owner, 2026-08-24): keep current behaviour in the port; spec the grace period as a later improvement.** `SetSecretRef` overwrites the single `client_secret_ref` column and `verifyClientSecret` has no fallback, so the old secret 401s the instant rotate returns — making rotation a synchronised fleet-wide redeploy with no rollback path. The improvement is two secrets with **both** a timed lapse (default grace, `graceSeconds: 0` reproducing today) and an explicit immediate revoke for the leaked-secret case, which is discovered after rotation more often than before it. Deferred **out of the port**: `docs/improvements.md`. |
