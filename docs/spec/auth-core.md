...
| — 2026-09-13 (Java-first ruling) | The anchor bypass in `require`/`requireAny` (`auth.go:409`, the row above's "deliberate and untouched") is **withdrawn**. Owner ruling: permissions always come from roles, at every tier — an anchor-scoped principal's `scope` now governs reach only (which tenants it may act for), never authority (what it may do). Every provisioned application service account is anchor-scoped, so this was a live hole: its least-privilege `platform:application-service` role was decorative and its client-credentials token could call every admin route. Java-first; Go mirrors from here. Full spec: `docs/spec/permissions-from-roles.md`. | `Checks.require`/`requireAny`/`requireUserAdmin`; every anchor-scoped test fixture across the suite (`docs/backlog.md`'s "every anchor-scoped principal passes every permission check" entry, now resolved). |
...
| **Superseded 2026-09-25** (`security-fixes-2026-09-24.md` S2.5, commits `6a06a7f0`, `477db983`; `RefreshRotation.java`): refresh rotation is now atomic — consume (`UPDATE … WHERE consumed_at IS NULL … RETURNING`), insert the replacement and mark-replaced run in one transaction, and the replacement issues only when the consume changed exactly one row, so a crash between steps cannot strand the user. A rotated-out token presented again within a 10 s replay leeway, while its family is still intact, rotates into a sibling instead of being treated as reuse (two concurrent presentations from one session racing itself both succeed). Superseded text follows, describing Go's mechanism (not what Java does): refresh rotation steps are **not** transactional: revoke → insert → mark-replaced are three statements; a crash between revoke and insert strands the user (must re-login); `MarkAsReplaced` is best-effort so a crash there loses reuse detection for that hop. | Raw pgx, no tx. | RO:57-90 |
