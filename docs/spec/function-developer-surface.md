...
| Command | Does |
|---|---|
| `fn init <dir> [--runtime jvm\|wasm] [--package <java.package>] [--name <artifactId>] [--manifest-only]` (`function-manifest-authoring.md` M3) | **local only — never contacts the platform.** Scaffolds a starter project derived from `examples/function-hello`: `pom.xml` (depends on `flowcatalyst-function-api`, `provided`, which fcdev **ships with the scaffold**: the jar and a minimal pom under the project's own `lib/m2`, a file repository the pom declares, versioned as the fcdev that carried it — owner ruling 2026-09-24, fcdev rather than a Maven repository; commit `lib/m2` with the project), one handler class in `--package` (default `com.example.fn`), `manifest.json` with `"$schema"` pointing at `<platform-url>/api/schemas/function-manifest.json` (the `--platform-url` global option, else the local fcdev's default) and a `platform` endpoint. `--runtime wasm` or `--manifest-only` writes `manifest.json` only — there is no Wasm project template. Refuses when any file it would write already exists (exit 1, listing them, writes nothing) |
...
| `fn config get\|set <address> [KEY=VALUE…]`, `fn secret set <address> <KEY>` (value from stdin or `--from-file`, **never an argument** — shell history), `fn secret list\|delete` | `set` on config is read-modify-write of the full map. `config set`/`secret set` also create the function on a 404 for its address, reusing `fn publish`'s `ensureFunctionExists` (`--manifest`, `--client`, `--no-create`; `function-backlog-2026-09-22.md` Unit F) — so `set` no longer needs `fn publish` run first; `config get`/`secret list` never create |
