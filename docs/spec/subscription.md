...
| `connectionId` | string, optional | ~~not a foreign key; validated to exist **only by sync** (`CONNECTION_NOT_FOUND`), not by admin create/update — **load-bearing or accident?**~~ **Superseded 2026-09-25** (`security-fixes-2026-09-24.md` S3.1, commit `b1ce6e55`): still not a foreign key, but admin create, and any admin update that changes it, now require the resolved connection — and, when the connection names a service account, that account — to be one the caller may sign with (`SigningReach`, §10 open question 13) |
...
| `serviceAccountId` | string, optional | ~~not validated~~ **Superseded 2026-09-25** (`security-fixes-2026-09-24.md` S3.1, commit `b1ce6e55`; `server/src/main/java/io/flowcatalyst/platform/serviceaccount/SigningReach.java`): admin create, and any admin update that changes the endpoint, the account or the connection, now require the account (named directly or through `connectionId`) to be one the caller may sign with — a super-admin may use any account; an account owned by an application may be used only by that application; an application caller may use only its own accounts; otherwise the caller's tenancy must cover the account's (anchor-tier account → anchor only, client-linked → every linked client). A value carried over unchanged is not re-checked and may be dangling. Named rows must exist when the caller is setting them |
...
13. Admin create/update do not check that `connectionId` exists; sync does. **K3**: sync now also resolves/checks `connectionCode`, with the namespace and scope rules in §6/§7. **Superseded 2026-09-25** (`security-fixes-2026-09-24.md` S3.1, commit `b1ce6e55`): admin create now checks that a named `connectionId`/`serviceAccountId` exists, and an admin update checks it whenever that update changes the endpoint, the account or the connection; both also check that the caller may sign with the resulting account (`SigningReach`, §1 rows above). An update that leaves `connectionId` unchanged still never re-checks it, so a stored, already-dangling reference is not caught retroactively.
