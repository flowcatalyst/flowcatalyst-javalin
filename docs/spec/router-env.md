...
| `FC_ROUTER_DEFERRAL_BUDGET` | not present, same reason | how many deferred messages one queue's consumer may have outstanding before it stops polling into pools that are all full (§1, §7; `Env.routerDeferralBudget`, `RouterManager#DEFAULT_DEFERRAL_BUDGET`). `0`/unset → implementation default (15000, raised from 5000 the same day — owner ruling 2026-09-22, `docs/go-mirror/2026-09-22-router-hol-deferral-handoff.md` addendum). Go carries the same knob (`ServerConfig.DeferralBudget`); Rust does not | `0` → 15000 |
...
| `FC_NOTIFY_BATCH_INTERVAL_SECONDS`, `NOTIFICATION_BATCH_INTERVAL` | `NOTIFICATION_BATCH_INTERVAL` only | same value, `FC_NOTIFY_BATCH_INTERVAL_SECONDS` canonical first added (`Env.routerNotifyBatchIntervalSeconds`); `0` on both sides means no batching — every notice its own card | `60` (owner ruling 2026-09-23; was 300) |
...
| `AUTH_MODE`, `FC_ROUTER_AUTH_USER`/`AUTH_BASIC_USERNAME`, `FC_ROUTER_AUTH_PASS`/`AUTH_BASIC_PASSWORD` | resolved in `fc_router::api::AuthConfig::from_env` (not `main.rs`) — `AUTH_MODE=NONE` (or unset) disables BasicAuth on the router surface. **Java 2026-09-25: honoured in dev mode only**; elsewhere ignored with a WARN, and the API needs a platform bearer token (`router-api-auth.md`) | same three names, same `NONE` behaviour, already implemented before this brief (`Env.routerAuthMode`/`routerAuthUser`/`routerAuthPass`) | `NONE` |
