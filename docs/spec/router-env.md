...
| `FLOWCATALYST_CONFIG_URL` | required unless `FLOWCATALYST_DEV_MODE=true`; comma-separated URLs supported | required to run the config-service mode; blank + `FC_DEFAULT_BROKER=postgres` runs the built-in Postgres broker instead of refusing to start (`Router.configSource`) — a **deliberate** difference: the Rust binary refuses to start with neither, Java degrades to no queues configured only when `FC_DEFAULT_BROKER` is not `postgres` either | none |
