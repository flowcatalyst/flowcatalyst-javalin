...
| `/api/applications/{appCode}/subscriptions/sync` | `SUBSCRIPTION_SYNC` \| `SUBSCRIPTION_MANAGE` \| `APP_SVC_SUBSCRIPTION_{CREATE,UPDATE,DELETE}`; the per-application (and, when `clientId` is given, per-client) access check lives in the operation's `authorize` phase (`SyncSubscriptionsCommand` carries an `applicationId`) | `SyncSubscriptionsRequest{clientId, subscriptions[]: {code*, name*, description, target*, connectionId, connectionCode, sharedConnection, eventTypes*[]: {eventTypeCode*, filter}, dispatchPoolCode, mode, maxRetries, timeoutSeconds, dataOnly}}` → `SyncSubscriptionsCommand(app.id, app.code, resolvedClientId, inputs, removeUnlisted)`; `dataOnly` absent ⇒ `false`; **K3** (`code-first-connections.md` §3): `clientId` is the client's id **or** its identifier slug, resolved to an id by the handler BEFORE the operation runs (unknown → 404 `Client_NOT_FOUND`), same as `connections/sync`; `connectionCode`/`sharedConnection` resolve a connection by code within an explicit namespace, no fallback | `API`/`CODE` subscriptions owned by `(applicationCode, clientId)` not listed are hard-deleted (**K3**: scoped to the request's client, `NULL` matching `NULL` only — previously every client's rows for the application) | `SyncResultResponse` |
...
**`subscriptions/sync` gained `clientId` in `code-first-connections.md` slice
K3**: the same `resolveClientRef` helper `syncSubscriptions` now shares with
`syncConnections` (both in `SdkSyncApi`) — no new route, no `State` change;
`clients` was already there for K2.
