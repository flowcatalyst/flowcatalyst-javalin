...
8. ~~**Unmapped email domain at `/auth/oidc/login`**~~ **RULED and BUILT 2026-09-25: 404 `EMAIL_DOMAIN_NOT_MAPPED`, logged at INFO**; a mapped domain whose IdP is broken stays 500. Supersedes auth-identity §4.3. The original question: it answers 500 `OIDC_RESOLVE_FAILED` (spec
   auth-identity §4.3, Go parity) — a user's typo, not a server fault. 400/404 instead?
9. ~~**Function outbound HTTP response cap**~~ **RULED and BUILT 2026-09-25: fixed 16 MiB**: a streamed read, failing with `RESPONSE_TOO_LARGE` to the function past the cap. Not configurable; documented in `functions.md`. The original question follows. — `AllowlistHttpCaller` reads the whole body; an
   allowlisted host returning GBs exhausts the host. A fixed cap changes the author contract.
10. ~~**Listener idle timeouts**~~ **RULED 2026-09-25: phase-aware, fixed values, both listeners.** A keep-alive idle connection closes after 75 s (above the ALB's 60 s). The request (headers and body) must be read within 30 s. No timeout while the handler runs or a response streams. **BUILT with one limit:** `KeepAliveIdle` closes a connection 75 s after its last request, on the platform (plain and TLS) and function-host (private and public) listeners. It never cuts in-flight work, and the existing per-request deadline bounds reading a body. A connection that never completes a *first* request cannot be seen: Vert.x surfaces an HTTP/1.1 connection only once that request's headers are in, and its transport idle timeout would also cut silent in-flight work. That case is the ALB's in production. If it must be closed in the process, the remaining option is a Netty pipeline handler, a bigger change. The original question follows. — neither the platform nor the function host sets one; a slow-body
...
11. ~~**Public API shapes**~~ **RULED 2026-09-25:** function-api `Events.emit` returns a sealed `EmitResult` (`Emitted(id)` / `Refused(code, status, message)`) and `EventEmitException` goes. This breaks only the operator's own functions, and the API ships with fcdev. The SDKs gain a result-returning webhook `check()` (`Valid` / `Invalid(reason)`) in Java, TS and Laravel, and `verify()` stays as the throwing wrapper (additive, minor bump). **BUILT 2026-09-25:** `EmitResult` in function-api (`HttpControlPlane` reads `results[0].id` into `Emitted`, and an unreadable 2xx body is still `Emitted("")` so an accepted event is never retried into a duplicate); the Wasm `fc_emit_event` answer gains `eventId`; the SDKs' `check()` in Java, TS and Laravel. Two follow-ups: (a) the **JS guest library** (`clients/function-js`) still throws `EventEmitException`, because changing it means rebuilding the committed `function_hello_js.wasm` fixture (the Rust guest already returns a `Result`); (b) a replayed `dedupId` is an idempotent `SUCCESS` carrying a freshly generated id, not the stored event's id, on this route and on `/api/events/batch` alike (Go's behaviour, kept); `Emitted.eventId` documents it. The original question follows. — `EventEmitException` (function-api: an expected outcome as an unchecked
...
16. ~~**`/bff/event-types/sync-platform` takes `applicationCode` from the body**~~ **RULED and BUILT 2026-09-25: always `platform`; a body naming any other application is 400 `PLATFORM_SYNC_ONLY`.** The original question: and syncs the
