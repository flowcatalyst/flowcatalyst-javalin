...
- ~~Client selection (`/auth/client/*`, the last unported Go route group)~~ **Done** (orchestrator; spec §6.5 written from Go). ~~Admin 2FA reset audit row~~ **Done**. ~~I-Q16 branding spelling~~ **Done**. ~~C4 reset-approvals~~ **Done** (Sonnet, `fe83fc5`). ~~Application `provision-*` (the last two lockfile operations)~~ **Done** (Sonnet, `ec55164`; coverage 245/245, threshold 1.0). Phase 5 under way.
