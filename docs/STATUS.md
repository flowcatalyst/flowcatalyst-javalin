...
- **W6 landed (2026-09-25): Rust functions** — `clients/function-rust` (`flowcatalyst-function`
  ...
  builders return `Result` too), `examples/function-hello-rust` (committed module, reproducible
  `--locked` build), `fn init --lang rust`, `WasmFunctionHelloRustTest`; docs `functions.md` §8c.
...
- **Owner rulings 2026-09-19/20 (R5–R11), spec `docs/spec/function-invocation.md`** (supersedes the
  trigger proposal): a function is **always invoked over HTTP** at
  `/functions/{address}[:{version}]/{function-path}` (version optional, never generated by the
  platform; private entry only); the manifest declares `endpoints` (each with a required `auth`:
  `webhook` | `platform` | `none`), `subscriptions`, `schedules`, `public`; the platform creates the
  subscriptions/jobs/pool at **promote** and deletes what the manifest no longer lists; subscription
  source **`FUNCTION`** — a deliberate divergence from Go (functions are post-cutover while Go shares
  the database; `cutover.md` §4.1); one dispatch pool per function; pool URL `FC_FN_POOL_URL`
  (`http://fn-{pool}:8080`).
- **Delivery signing landed on `main`** (`41d718cb`, spec `dispatch-delivery-credentials.md`; local,
  not pushed) and is merged here: dispatch-job webhooks were going out unsigned.
- **Landed here:** I1 manifest reshape + `FUNCTION` source + sync protection (pool and scheduled-job
  SDK syncs would have archived function-owned objects); I2 wiring reconciliation (`dde7b963`);
  I3 API jar is HTTP-only — `Request`/`Result`/`Caller`/`Webhook` (`fefb39c5`); D1 loader core;
  D2 reconciler (`dbdc10a1`).
