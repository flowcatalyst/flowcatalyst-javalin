...
- **Router API authentication (2026-09-25, `docs/spec/router-api-auth.md`):** outside dev mode the router's API (monitoring, `/messages`, breaker resets, …) needs a platform bearer token holding `platform:messaging:router:view` or `:operate`. Assign the new `platform:router-operator` role to the people and service accounts that operate the router; `platform:viewer` and `platform:application-service` hold `:view`. Tokens are verified against the platform at `FC_ROUTER_PLATFORM_URL`, so without it every call but health, metrics and the dashboard page answers 401. `AUTH_MODE=NONE` and `FC_ROUTER_AUTH_USER`/`_PASS` are ignored outside dev mode, with a WARN each; drop them. **Dashboard sign-in:** register a **public** OAuth client on the platform with API access, the `platform` application, and the dashboard page's URL (`https://{routerDomain}{prefix}/dashboard.html`) as its redirect URI, and give its client id to the router as `FC_ROUTER_DASHBOARD_CLIENT_ID`. The mock routes (`/api/test/*`, `/api/benchmark/*`, `/api/seed/messages`) exist in dev mode only, so a load-test router needs `FLOWCATALYST_DEV_MODE=true`. **Roll-out:** ship the SDK release whose router resource sends the bearer token before deploying a router that enforces this.
